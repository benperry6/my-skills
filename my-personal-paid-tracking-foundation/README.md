...
- pour un site possede en propre, la base search doit aussi inclure `Bing Webmaster Tools`, avec une verification canonique du domaine par `TXT DNS` chez le registrar quand on veut couvrir tout le domaine, mais le bootstrap BWT ne doit etre traite comme reutilisable que lorsqu'il a ete valide en comportement reel
