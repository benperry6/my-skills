...
- AI control of `Chrome` / `Brave` must remain possible, including a browser restart when required, but only through a session-safe procedure that preserves and verifies the user's real tabs, windows, profiles, cookies, and logins
...
- Default `Chrome AI-safe` startup launches the cloned profile in parallel without quitting, restarting, or activating the user's real Chrome session. If direct control of the real Chrome/Brave session requires a restart, that restart is allowed only after a reliable pre-restart snapshot and only if the post-restart state is verified against the expected user session; never silently replace, regress, or pollute the user's real session just to expose CDP.
- A persistent Chrome real-session guardian auto-discovers every real Chrome profile under `~/Library/Application Support/Google/Chrome` (`Default` and all `Profile *` directories), including profiles created later. It snapshots each profile's `Sessions` folder while Chrome is running, storing the latest known-good state under `~/.codex/browser-session-snapshots/chrome-profiles/<profile>/` (`Profile 3` keeps the legacy path `~/.codex/browser-session-snapshots/chrome-profile3`). If Chrome is closed and a profile's session files differ from that latest good snapshot, the guardian backs up the current files under `pre-restore/` and restores the latest snapshot before the next manual launch can reopen old tabs. This exists because the AI-safe mirror protects AI-controlled Chrome state, but a native Chrome crash can still leave a real profile pointing at stale session files.
- The Chrome real-session guardian must not restore or modify the real profile while any Chrome instance is running, including `Chrome AI-safe`; this prevents the guard and the AI-safe mirror from fighting over the same source profile.
...
- `~/.codex/browser-session-snapshots/chrome-profiles/*/latest`
