...
- While `Chrome AI-safe` is running, a background mirror copies the clone's live session-restore artifacts and auth/session storage artifacts (`Cookies`, `Local Storage`, `Session Storage`, `IndexedDB`, `Service Worker`, `Web Data`, etc.) back into the real Chrome profile, but only when the real Chrome app is not running. This prevents the user's next manual Chrome launch from reopening older tabs or losing accounts they used while Chrome was under AI-safe control.
